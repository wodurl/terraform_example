...
resource "aws_launch_template" "lt" {
  ...
  # ami 선택(우리가 직접 만든 ami를 선택할 수도 있다)
  # 우리가 미리 만들어 놓은 ami 중에 검색을 하여
  # 제일 최신의 ami를 사용하도록 설정할 예정
...
resource "aws_autoscaling_group" "asg" {
  ...
  launch_template {
    ...
    # 항상 최선의 이미지를 사용하도록(항상 최신의 Template을 사용하도록)
    version = "$Latest"
  }
...
resource "aws_autoscaling_policy" "cpu_scaling_policy" {
  ...
  target_tracking_configuration {
    ...
    # 50%를 넘어가면 -> scale out -> ec2 갯수가 늘어남
    # 50% 아래로 떨어지면 -> scale in -> ec2 갯수가 줄어듬
    target_value = 50
  }
}