...
resource "aws_autoscaling_group" "asg" {
  ...
  # 어떤 기준 때문에 갯수를 줄인다면 가동되어야 하는 최대 갯수
  min_size = var.min_size
  # 위에서 만든 template 정봅를 등록한다
  launch_template {
    id = aws_launch_template.lt.id
    # launch template의 소스 코드가 바뀌어서 버전 번호가 올라가면 바로 감지할 수 있도록 한다.
  ...
  instance_refresh {
    ...
    preferences {
      ...
      # 새 인스턴스가 뜨고 나서 다음 인스턴스를 교체하기 전까지 걸리는 시간
  ...
  default_cooldown = 60
  # ALB의 target group arn을 여기에 등록한다.
