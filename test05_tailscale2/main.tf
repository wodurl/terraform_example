# /home/user12/terraform_example/test05_tailscale2
...
resource "aws_route_table" "public_rt" {
    vpc_id = aws_vpc.main.id
    # vpc 대역이 아닌 다른 모든 ip에 대한 요청은 internet gateway로 보내겠단 의미
    route {
...
resource "aws_route_table" "private_rt" {
    ...
    route {
        cidr_block     = "0.0.0.0/0"
        # 위에서 만든 NAT Gateway로 나가도록 설정
        nat_gateway_id = aws_nat_gateway.nat_gw.id
    }
}

# private subnet과 private routing table을 연결
...
# ----------------------------------------------------
# Public Subnet에서 172.16.8.0/24로 갈 때 EC2를 거치도록 설정 (my_ec2는 아래에 정의할 예정)

# aws_route는 미리 만들어 놓은 aws_route_table에 라우팅 정보를 추가하는 용도로 사용할 수 있다.
...
resource "aws_instance" "my_ec2" {
    ...
    # 대장 ec2가 만들어지는 시점에 tailscale에 접속해서 vpn 연결을 활성화하는 작업을 script 구현하겠다
    # 조금 복잡하지만 다음 예제에서는 이걸 tailscale terraform
    user_data = <<-EOF
        #!/bin/bash
        # 로그 파일 생성 및 모든 출력 기록
