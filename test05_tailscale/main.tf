...
terraform {
    ...
    backend "s3" {
        ...
        key = "lecture/terraform.tfstate" # /lecture/하위에 만들어지도록
...
resource "local_file" "ssh_key" {
  ...
  filename                      = "${path.module}/lecture_key.pem"
...
resource "aws_instance" "my_ec2" {
    ...
    # 다른 ec2에서 mgmt 쪽으로 보내는 traffic을 전달할 수 있도록
    source_dest_check = false # 이 설정을 false로 해야 라우팅 기능을 수행할 수 있음
    tags = {
        Name = "my-ec2"
    }
    # ec2가 provision될 때 실행할 script를 지정할 수 있다.
    user_data = <<-EOF
        #!/bin/bash
        ...
        all = {
            hosts = {
                "${aws_instance.my_ec2.public_ip}" = {
                    ...
                    ansible_ssh_private_key_file = "${path.module}/lecture_key.pem"
                }
            }
        }
    })
...
resource "terraform_data" "ansible_run" {
    ...
    triggers_replace = {
        ...
        always_run = "${timestamp()}" # 실행할 때마다 시간이 달라지므로 무조건 플레이북이 실행된다.
    }

    # 아래의 작업이 성공한 기억이 있으면 다시 또 실행되지 않는다.
    provisioner "local-exec" {
        # DB 설치, 파일 copy, 권한 변경 등이 잦으면 에러가 날 수 있음
        # 안정적으로 실행하기 위해 ANSIBLE_SSH_PIPELINE=1 이라는 임시 환경변수 추가
        # 해당 환경변수는 ansible playbook이 실행될 때에 한해 ansible이 활용하는 일시적인 환경변수이다. 
        command = "ANSIBLE_SSH_PIPELINE=1 ansible-playbook site.yml --extra-vars 'host_name=${var.host_name}' "
    }
}
